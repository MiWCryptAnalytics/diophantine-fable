...
\date{Draft of 2026-07-22, assembled from the post-referee notes (v2);
  all referee-mandated corrections applied; the numeric evaluation of
  one absolute constant is deferred and flagged
...
prior (Smart, ANTS-II 1996) is a per-method practical estimate with
heuristic unit supply. The method walks the rank-one unit orbit of
...
instances; an independent referee reimplementation matches 13/13,
including three high-index fields. All constants are explicit modulo
...
\item \textbf{Smart} \cite{Sma96} is the closest prior and the work we
  position against. It is a complexity \emph{estimate} for the
  Tzanakis--de Weger method at fixed degree $d$, measured in
  $\log|m| + \log L(F)$: a small-solutions phase of cost
  $O(|m|^{1/(d-2)})$ (exponential in the input size; $O(|m|)$ for
  cubics) and a reduction phase polynomial in the regulator and the
  input --- with fundamental units supplied by then-heuristic
  subexponential algorithms, and with explicitly noted practical
  failure modes (large $m$, large regulator or class number,
  near-coincident roots).\footnote{The exponent shapes attributed to
    \cite{Sma96} are quoted from the zbMATH review (Zbl 0896.11009)
    and a secondary rendering; the primary text is paywalled, and the
    exact exponents should be re-verified against it before
    submission.} Smart's own conclusion: ``we seem to be a long way
  off from a `good' algorithm''. It is a per-method,
  heuristic-unit-supply, practical estimate --- not a rigorous
  worst-case decision theorem.
...
book, Hanrot's LLL survey, Evertse--Gy\H{o}ry, Waldschmidt, Ga\'al,
Kim, Gherga--Siksek) found effectivity or practicality statements only.
...
The technical heart is Step 6. A black-box application of Sha's
explicit Skolem-type theorems \cite{Sha19} gives a window of size
$e^{22.5R}$ --- which is $2^{O(s)}$ only under a \emph{redefined} size
measure containing $R$ itself; in true input size it is doubly
exponential, since $R = 2^{\Theta(s)}$ (Remark~\ref{rem:blackbox}
preserves this near-miss as a cautionary lesson). The repair is not a
...
covers it; plain exact products along the cycle suffice (Lemma A$'$),
compact representations being a luxury needed only for poly-size
certificates, which we do not pursue.
...
U, A$'$, C and Proposition F), with all referee-mandated corrections
applied. Section~\ref{sec:proofmain} assembles the proof of
...
of this work got it wrong, and an adversarial referee falsified the
naive ``clear by 3'' claim at 91 values of $d < 500$ (e.g.\ $d = 12,
45, 175$) --- that clearing by $3$ alone is \textbf{not} sufficient:
the correct clearing constant is $3b$.
...
At unit rank one the class group is never needed: principality of
...
(Lemma U, referee-sharpened form): the reduced representative
$\gamma'$ satisfies
\[
...
\begin{proof}[Proof (with the referee-corrected orientation and
certification)]
...
\item \textbf{Why plain products.} An earlier plan routed height
  ...
  referee's simplification, adopted here: at the $2^{O(s)}$ budget
...
Note the phenomenon the referee flagged and the empirical data
confirm: after unit reduction the $R$'s cancel --- the forward window
...
\begin{itemize}
\item \emph{Step 1:} $O(R\,D^\epsilon) = 2^{O(s)}$ bit-operations
...
close it). For completeness: \texttt{bnfisintnorm} returns a complete
...
\subsection{The referee's independent reimplementation: 13/13}
\label{sec:referee}

The hostile-referee pass that produced the corrections recorded in
Section~\ref{sec:lemmas} also built its \textbf{own from-scratch
mini-implementation} of the algorithm and validated it against
certified \texttt{thue}: \textbf{13/13 exact agreements}, including
the high-index fields
\[
d = 12,\ 45,\ 175 \qquad
([\,O_K : \mathbb{Z}[\alpha]\,] = 2,\ 3,\ 5),
\]
precisely the fields at which the referee had falsified the earlier
``clear by 3'' claim (Section~\ref{sec:index}) --- so the validation
deliberately covers the corrected code path. The same pass verified
the linear-form analysis of Lemma C numerically on two fields and
